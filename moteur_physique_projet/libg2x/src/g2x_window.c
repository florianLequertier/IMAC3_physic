...
extern void g2x_SelectDrawWin(void)
{
...
static void (*g2x_Calc)(void)=NULL;             /*! fonction de dessin        !*/
...
/*= - BkgFunc   : procedure d'affichage "background" : non modifi̩             =*/
...
/*= - CalcFunc   : procedure d'affichage, associee a <glutPostRedisplay>             =*/
extern void g2x_SetCalcFunction(void (*f)(void)) { g2x_Calc = f; }
...
static __inline__ void g2x_DrawDisplay(void)
{
  ...
  if (_GRID_) g2x_GridDft();
	...
	if (G2Xdisplayflag && g2x_Calc) { g2x_Calc(); G2Xdisplayflag=false; }
	g2x_Draw(); 
...
static __inline__ void g2x_CornMouseClic(int button, int state, int x, int y)
{
  ...
  if (pushmode!=GLUT_UP) return;
	...
	glutPostRedisplay();
	glutShowWindow(); 
	return (G2Xdisplayflag=true,glutPostWindowRedisplay(drawwin));
}
  ...
  while (scroll<_SCROLLV_+scrollvnum && pushmode==GLUT_UP)
  {
                                            ...
                                            && y<(Yscrollend  +2*scroll->w) )
    {
      ...
      TheScrollv->cursor= (double)(TheScrollv->ycurs-Yscrollstart)/(double)Yscrollrange;
			...
			glutPostRedisplay();
			glutShowWindow(); return (G2Xdisplayflag=true,glutPostWindowRedisplay(drawwin));
    }
    scroll++;
  }
  return;
...
{ 
  ...
  switch (moving)
  {
    case true+GLUT_LEFT_BUTTON   :
      ...
      (G2Xdisplayflag=true,glutPostWindowRedisplay(drawwin));
      return;
  }
} 
